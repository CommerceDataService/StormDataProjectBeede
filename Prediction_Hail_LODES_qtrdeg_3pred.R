...
# uses LODES data and 1/4 degree lat/lon rounding
setwd("~/StormProject")
library(dplyr)
library(glmnet)
...
load("SWDI_SE_ACS_LODES.rda")
SWDI_SE_ACS_LODES <- select(SWDI_SE_ACS_LODES, inSE, #Tract_popdens, #populaton density
       #County_popdens,
...
set.seed(123)
numobs <- dim(SWDI_SE_ACS_LODES)[1]
SWDI_SE_ACS_LODES$rand1 <- runif(numobs)
SWDI_SE_ACS_LODES$id <- 1:numobs
#Splitting
  split <- 0.7
  train <- SWDI_SE_ACS_LODES[SWDI_SE_ACS_LODES$rand1 <= split, ]
  test <- SWDI_SE_ACS_LODES[SWDI_SE_ACS_LODES$rand1 > split, ]
...
#k <- interval
  ...
  save(voting_list, file = "Prediction_Hail_LODES_qtrdeg_3pred.rda")
