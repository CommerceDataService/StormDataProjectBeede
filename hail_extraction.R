...
swdi_pull <- function(start_date,end_date,series){
  ...
  for(i in seq(0,range,10)){
    ...
    ##Cut down bounding box to limit data to Nebraska
      ##Nebraska latitude is 37 degrees North by 40 degrees North
      ##Nebraska longitude is 94 deg 35 min West by 103 deg 3 min W
      ##equals -94.583 by -103.050 (divide minutes by 60)
      raw <- raw[raw$LON <= (-94.583) & raw$LON >= (-103.050) 
...
save(raw, file = "Nebraska_extract.rda")
