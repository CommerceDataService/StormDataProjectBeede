...
SData <- mutate(SData, EventTypeIsHail = grepl("HAIL", EVENT_TYPE),
                 NarrativeIsHail = grepl("HAIL", EVENT_NARRATIVE)
#                        | grepl("HAIL", EPISODE_NARRATIVE) 
                        )
table(SData$EventTypeIsHail, SData$NarrativeIsHail)

selectdata <- select(SData, EVENT_TYPE, EVENT_NARRATIVE, 
                     EventTypeIsHail, NarrativeIsHail)
selectdatafil <- filter(selectdata, EventTypeIsHail == TRUE | NarrativeIsHail == TRUE)
testdata <- selectdatafil[selectdatafil$EventTypeIsHail == FALSE 
                          & selectdatafil$NarrativeIsHail == TRUE,]
