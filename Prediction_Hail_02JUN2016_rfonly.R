    ...
    #Inner loop # Insert classifier here
      ...
      #     test_results <- cbind(test_results,prob1)
       ...
       #CLASSIFIER 2:  random forests
          ...
          #CV
          #cvfit = rfcv(trainx=x, trainy=y, ntree=100)
          #plot(cvfit$n_var, cvfit$error.cv)

          #Predicted

          #prob2 = as.numeric(predict(fit, newdata = test_mat, type = "prob")) 
          prob2 = predict(fit, newdata = test_mat, type = "response")   
          prob2 <- as.numeric(prob2) - 1
          #Collect the results on the test set
