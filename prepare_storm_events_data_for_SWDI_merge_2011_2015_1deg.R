...
for (year in 2011:2015) {
  ...
  merged_SE_1deg <- mutate(merged_SE, EVTYPEcap = toupper(EVENT_TYPE))
  merged_SE_1deg <- merged_SE_1deg[grep("HAIL", merged_SE_1deg$EVTYPEcap),]
  print(paste("number of rows of hail-only events = ", dim(merged_SE_1deg)[1], sep=""))
  summary(merged_SE_1deg$MAGNITUDE)
  print(paste("no. obs w/missing hail size = ", sum(is.na(merged_SE_1deg$MAGNITUDE)),
              sep=""))
  # table(merged_SE_1deg_HAIL$STATE)
  # table(merged_SE_1deg_HAIL$STATE_FIPS)
  # sum(is.na(merged_SE_1deg_HAIL$STATE_FIPS))
  
  #Cut down bounding box to continental US not including Alaska
  merged_SE_1deg <- merged_SE_1deg[merged_SE_1deg$LONGITUDE<(-50) & merged_SE_1deg$LONGITUDE>(-140) & 
                           merged_SE_1deg$LATITUDE > 25 & merged_SE_1deg$LATITUDE <= 49,]
  
  print(paste("number of rows after limit to lower 48 states = ", 
              dim(merged_SE_1deg)[1], sep=""))
  ...
  print(paste("no. obs with begin lat != end lat = ",
              sum(merged_SE_1deg$BEGIN_LAT != merged_SE_1deg$END_LAT), sep=""))
  print(paste("no. obs with begin lon != end lon = ",
              sum(merged_SE_1deg$BEGIN_LON != merged_SE_1deg$END_LON), sep=""))

  #assess extent to which LONGITUDE & LATITUDE differ from BEGIN/END LAT and LON
  LAT_LON <- select(merged_SE_1deg, EPISODE_ID, EVENT_ID, BEGIN_LON, BEGIN_LAT, END_LON, 
  ...
  begin_mat <- as.matrix(select(merged_SE_1deg, BEGIN_LON, BEGIN_LAT))
  end_mat <- as.matrix(select(merged_SE_1deg, END_LON, END_LAT))
...
#create number of hours to adjust date/times to GMT  
  ...
  x <- date(merged_SE_1deg$new_end_time) - date(merged_SE_1deg$new_beg_time)
  x <- as.period(x, "days")
  x <- sort(x)
  print("assessment of whether it is worthwhile to reshape data by beg vs end times")
  print(tail(x, 50))
  
  ##Does one degree make the most sense in this context?
  #set rounding factor for lat/lon
  int = 1
  ##Round coordinates
  merged_SE_1deg <- mutate(merged_SE_1deg, BEGIN_LON = round(BEGIN_LON/int)*int,
  ...
  print(paste("no. of missing direct injuries =", 
              sum(is.na(merged_SE_1deg$INJURIES_DIRECT)), sep=" "))
  print(paste("no. of missing indirect injuries = ", 
              sum(is.na(merged_SE_1deg$INJURIES_INDIRECT)), sep=" "))

  merged_SE_1deg <- mutate(merged_SE_1deg, 
                      injuries = as.numeric((INJURIES_DIRECT + INJURIES_INDIRECT) != 0))
  print(paste("no. of missing direct deaths = ", 
              sum(is.na(merged_SE_1deg$DEATHS_DIRECT)), sep=""))
  print(paste("no. of missing indirect deaths = ", 
              sum(is.na(merged_SE_1deg$DEATHS_INDIRECT)), sep=""))
  merged_SE_1deg <- mutate(merged_SE_1deg, 
                      deaths = as.numeric((DEATHS_DIRECT + DEATHS_INDIRECT) != 0))
  merged_SE_1deg <- mutate(merged_SE_1deg, DAMAGE_PROPERTY = as.character(DAMAGE_PROPERTY),
                      DAMAGE_CROPS = as.character(DAMAGE_CROPS))
  merged_SE_1deg <- mutate(merged_SE_1deg, property = as.numeric((substr(DAMAGE_PROPERTY, 1, 4) != "0.00")
                      & (DAMAGE_PROPERTY != "") & (DAMAGE_PROPERTY != " ")))
  merged_SE_1deg <- mutate(merged_SE_1deg, crops = as.numeric((substr(DAMAGE_CROPS, 1, 4) != "0.00")
                      & (DAMAGE_CROPS != "") & (DAMAGE_CROPS != " ")))
  merged_SE_1deg <- mutate(merged_SE_1deg, anydamage = as.numeric((injuries == 1) | (deaths == 1) | 
                                               (property == 1) | (crops == 1)))
  ...
  print(merged_SE_1deg[1:50, titlelist])
  
  #dedupe the data by DATE, LAT, LON
  merged_SE_1deg <- merged_SE_1deg %>% group_by(DATE, BEGIN_LON, BEGIN_LAT, 
                                      END_LON, END_LAT) %>% 
  ...
  merged_SE_1deg <- ungroup(merged_SE_1deg)
  merged_SE_1deg <- as.data.frame(merged_SE_1deg)
  print("dimension of data fram after de-duping")
  print(dim(merged_SE_1deg))
  
  #split dataframe in one where begin and end points are the same and one where diff
  merged_SE_1deg_same = merged_SE_1deg[(merged_SE_1deg$BEGIN_LAT == merged_SE_1deg$END_LAT) &
                               (merged_SE_1deg$BEGIN_LON == merged_SE_1deg$END_LON),]
  merged_SE_1deg_same = merged_SE_1deg_same[, -c(4:5)]
  merged_SE_1deg_same = rename(merged_SE_1deg_same, LON = BEGIN_LON, LAT = BEGIN_LAT)
  
  print("str(merged_SE_1deg_same")
  print(str(merged_SE_1deg_same))
  merged_SE_1deg_diff = merged_SE_1deg[(merged_SE_1deg$BEGIN_LAT != merged_SE_1deg$END_LAT) |
                               (merged_SE_1deg$BEGIN_LON != merged_SE_1deg$END_LON),]
  
  # merged_SE_1deg_diff$BEGIN_LON_OLD=merged_SE_1deg_diff$BEGIN_LON
  # merged_SE_1deg_diff$BEGIN_LAT_OLD=merged_SE_1deg_diff$BEGIN_LAT 
  # merged_SE_1deg_diff$END_LON_OLD=merged_SE_1deg_diff$END_LON 
  # merged_SE_1deg_diff$END_LAT_OLD=merged_SE_1deg_diff$END_LAT
  merged_SE_1deg_diff$id <- rownames(merged_SE_1deg_diff)
  print("str(merged_SE_1deg_diff")
  print(str(merged_SE_1deg_diff))
  ...
  for(k in 1:nrow(merged_SE_1deg_diff)){
  
  #  print(k)
    
    #get the interpolated line
    temp <-step(merged_SE_1deg_diff,2,4,3,5,k,id)
  #  print(temp)
  ...
  merged_SE_1deg_diff <- merged_SE_1deg_diff[,-c(2:5)]
  merged_SE_1deg_diff <- merge(merged_SE_1deg_diff, batched, by.x=c("id"),by.y=c("id"))
  merged_SE_1deg_diff <- select(merged_SE_1deg_diff, -id)
  print("str of merged_SE_1deg_diff")
  print(str(merged_SE_1deg_diff))
  
  
  merged_SE_1deg <- rbind(merged_SE_1deg_same, merged_SE_1deg_diff)
  print("str of merged_SE_1deg after recombining with same and diff dfs")
  print(str(merged_SE_1deg))
  
  merged_SE_1deg$inSE <- 1
  print(summary(merged_SE_1deg))
  print("table of minanydamage vs. maxanydamage")
  print(table(merged_SE_1deg$minanydamage, merged_SE_1deg$maxanydamage))
  save(merged_SE_1deg, file = paste("DeDuplicateHailStormEventData_",year, "_1deg", ".rda",
        sep = ""))
}